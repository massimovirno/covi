        ...
        {if $dati != false}
            ...
            {section name=i loop=$dati}
                {if $smarty.section.i.iteration % 2 == 1}
                    ...
                    <h1><a href="?controller=evento&task=dettagli&id_evento={$dati[i].eventoID}">{$dati[i].nome}</a></h1>
                    <h5>Vino: {$dati[i].vino}</h5>
                    <p><img height="120" src="foto/{$dati[i].foto}" alt="{$dati[i].nome}" title="{$dati[i].nome}">{$dati[i].descBreve|truncate:240:" [...]"}</p>
                    ...
                    <p class="details"><b>Media Voti:</b> | <a href="#">{if $dati[i].media_voti>0}{$dati[i].media_voti}{else}-{/if}</a> | Prezzo: <a href="#">{$dati[i].prezzo|string_format:"%.2f"}</a> |</p>
                        <form action="index.php" method="post">
                            <input type="hidden" name="id_vino" value="{$dati[i].vinoID}" />
            ...
            {section name=i loop=$dati}
                {if $smarty.section.i.iteration % 2 == 0}
                    ...
                    <div class="content-2col-box">
                        <h1><a href="?controller=evento&task=dettagli&id_evento={$dati[i].eventoID}">{$dati[i].nome}</a></h1> 			
                        <h5>Vino: {$dati[i].vino}</h5>
                        <p><img height="120" src="foto/{$dati[i].foto}" alt="{$dati[i].nome}" title="{$dati[i].nome}">{$dati[i].descrizione|truncate:240:" [...]"}</p>
                        {assign var="somma" value="`0`"}
                        {assign var="max" value="`0`"}
